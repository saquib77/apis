...
public class UserController {
	...
	private ResponseEntity<Optional<List<User>>> getAllUser(){
		...
		if(users.get().isEmpty()) {
			return ResponseEntity.status(HttpStatus.NO_CONTENT).build();	// no users
		}
		return ResponseEntity.status(HttpStatus.OK).body(users);		// have users
	}
	...
	private ResponseEntity<Optional<User>> getUserByUUID(@PathVariable("id") UUID id){
		...
		if(userid.isPresent()) {	
			return ResponseEntity.status(HttpStatus.OK).body(userid);		// getting user by id
		}
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();		// user does'nt exist
	}
	...
	private ResponseEntity<Optional<User>> getUserByEmail(@RequestParam String email){
		...
		return ResponseEntity.status(HttpStatus.OK).body(userid);			//	getting user by email
	}
	...
	private ResponseEntity<User> registerUser(@RequestBody User user){
		...
		if(user!=null) {
			...
			return ResponseEntity.status(HttpStatus.CREATED).body(userdb.get());	// user registered
		}
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();			// request body is null
	...
	private ResponseEntity<User> updateUser(@RequestBody User user){
		...
		if(userdb.isPresent()){
			return ResponseEntity.status(HttpStatus.OK).body(userdb.get());			// update user
		}
	...
	private ResponseEntity<User> deleteUser(@PathVariable("id") UUID id){
		...
		if(userStat.isPresent()) {
			return ResponseEntity.status(HttpStatus.OK).body(userStat.get());			// deleting user
		}
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
	}
